...
final class FragTestManagerState implements Parcelable {
    ...
    public static final Parcelable.Creator<FragTestManagerState> CREATOR
            = new Parcelable.Creator<FragTestManagerState>() {
...
final class FragTestManagerImpl extends FragTestManager implements LayoutInflaterFactory {
    ...
    public void dump(String prefix, FileDescriptor fd, PrintWriter writer, String[] args) {
        ...
        if (mActive != null) {
            N = mActive.size();
            if (N > 0) {
                writer.print(prefix); writer.print("Active FragTests in ");
        ...
        if (mAdded != null) {
            N = mAdded.size();
            if (N > 0) {
                writer.print(prefix); writer.println("Added FragTests:");
                for (int i=0; i<N; i++) {
        ...
        if (mCreatedMenus != null) {
            ...
            if (N > 0) {
                writer.print(prefix); writer.println("FragTests Created Menus:");
                for (int i=0; i<N; i++) {
                    FragTest f = mCreatedMenus.get(i);
                    writer.print(prefix); writer.print("  #"); writer.print(i);
                    writer.print(": "); writer.println(f.toString());
                }
            }
        ...
        writer.print(prefix); writer.println("FragTestManager misc state:");
    ...
    public boolean dispatchCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        boolean show = false;
        ArrayList<FragTest> newMenus = null;
        if (mAdded != null) {
            for (int i=0; i<mAdded.size(); i++) {
                FragTest f = mAdded.get(i);
                if (f != null) {
                    if (f.performCreateOptionsMenu(menu, inflater)) {
                        show = true;
                        if (newMenus == null) {
                            newMenus = new ArrayList<FragTest>();
                        }
                        newMenus.add(f);
                    }
                }
